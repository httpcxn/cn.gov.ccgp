...
public class Hebei {
    public Logger logger = LogManager.getLogger(Hebei.class.getName());
    public void heBei(){
        logger.debug("开始处理 河北");
        ArrayList<Ccgp> list = new ArrayList<Ccgp>();
        String[] urlList = Constant.hebei;
        for (int i = 0; i < urlList.length; i++) {
            list = analysis(urlList[i],list);
        }
        HebeiDao hebei=new HebeiDao();
        hebei.save(list);
        logger.debug("结束处理 河北");
    }
